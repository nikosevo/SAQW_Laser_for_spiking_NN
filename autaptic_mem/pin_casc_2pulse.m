...
for Vabs = 5:1:5
    ...
    for Pin = pin
        ...
        [peaks1,loc] = findpeaks(Pout1(400000:end),'MinPeakProminence',0.025,'MinPeakHeight',0.02);
        if(~isempty(peaks1))
    ...
    spikes_cnt = max(find(init_peaks(:,length(pin))));
    ...
    subplot(1,4,1);
    for i = 1:spikes_cnt
        plot(pin*1e3,init_peaks(i,:)*1e3,'Color',colors(mod(i,6) + 1,:),"LineWidth",2);
        hold on;
    end
    ...
    subplot(1,4,[2,4]);
    for i = 1:spikes_cnt
        plot(pin*1e3,pks1(i,:),'Color',colors(mod(i,6)+1,:),'LineWidth',3,'DisplayName',['Spike n.' num2str(i) '-Master']);
        hold on;
        
        plot(pin*1e3,pks2(i,:),'LineStyle','--','Color',colors(mod(i,6)+1,:),'LineWidth',3,'DisplayName',['Spike n.' num2str(i) '-Slave'])
        hold on;
    end
