...
for Vabs = 1
    ...
    for Pin = pin
        ...
        [peaks1] = findpeaks(Pout1,'MinPeakProminence',0.025,'MinPeakHeight',0.02);
        if(~isempty(peaks1))
    ...
    subplot(1,4,1);
    plot(pin*1e3,init_peaks*1e3,'Color',colors(mod(Vabs,6)+1,:),"LineWidth",3);
    hold on;

   

    subplot(1,4,[2,4]);
    plot(pin*1e3,pks1,'Color',colors(mod(Vabs,6)+1,:),'LineWidth',3);
    hold on;
    plot(pin*1e3,pks2,'LineStyle','--','Color',colors(mod(Vabs,6)+1,:),'LineWidth',3)
    ...
    legend('Vabs 1' , 'Vabs 1 Slave' , 'Vabs 2' , 'Vabs 2 Slave' , 'Vabs 3', 'Vabs 3 Slave' ,'Vabs 4', 'Vabs 4 Slave' ,'Vabs 5' , 'Vabs 5 Slave' ,'Location','NorthWest', 'FontSize' , 20 );
