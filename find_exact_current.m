...
% for Vabs = 2 I_bias lasing = 190mA spiking starts at 25
%to find the exact current just run the simulation from 24-25 with step e-5
spikes = zeros(length(ibs),1);
%figure('Renderer', 'painters', 'Position', [5 5 900 900]);
for Vabs = 0 : 5
    I_bias = I_bias_map(Vabs+1);
    ibs = I_bias- 1e-3 : 1e-5 : I_bias + 1e-3;
    ...
    for I_bias = ibs
        ...
        [peaks,~,w,~] = findpeaks(Pout,'MinPeakProminence',0.01);

        if(length(peaks)>0 && threshold == 0)
            threshold = I_bias;
        end 
        spikes(counter)= length(peaks);
        counter = counter + 1;
    ...
    legend( ['I bias threshold = ' num2str(threshold*1e3) 'mA'],'Location', 'northwest' , 'FontSize' , 20 )
    title(['Vabs = ' num2str(Vabs) ' Volt'])

    exportgraphics(gcf,['writting/chapter1/I_biasThreshold/Vabs' num2str(Vabs) '_exact.png'])
